<div id="head_wrap" class="clearfix">
	<div id="logo"><a href="#"><img src="images/hct_logo.gif" /></a></div>
    <!--<ul id="nav">
        <li id="btn_findPlan"><a href="#" title="Find a Plan">Find a Plan</a></li>
        <li id="btn_applyCoverage"><a href="#" title="Apply for Coverage">Apply for Coverage</a></li>
        <li id="btn_findSubsidy"><a href="#" title="Find a Subsidy">Find a Subsidy</a></li>
        <li id="btn_getAssistance"><a href="#" title="Get Assistance">Get Assistance</a></li>
        <li id="btn_learnMore"><a href="#" title="Learn More">Learn More</a></li>
    </ul>--><!--END NAV-->
</div><!--END head_wrap-->